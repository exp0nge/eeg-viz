...
pearson_band = np.load('band_full_pearson.dump')
print "Dump loaded"
abs_val = []
zero = []
...
plt.title('Sorted Correlations between 2 Channels')
plt.xlim(-1, len(sorted_pearson) + 1)
plt.ylim(-1.1, 1.1)
plt.xlabel('Channel vs Channel')
plt.ylabel('Correlation value')
# plt.xticks(range(64), labels)
plt.savefig('sorted_band_full_pearson.svg')
plt.show()


# sorted abs val
plt.plot(range(len(sorted_abs_val)), sorted_abs_val)
plt.title('Sorted Absolute Value Correlations between 2 Channels')
plt.xlim(-1, len(sorted_pearson) + 1)
plt.ylim(0, 1.1)
plt.xlabel('Channel vs Channel')
plt.ylabel('Correlation value')
# plt.xticks(range(64), labels)
plt.savefig('sorted_absolute_value_band_full_pearson.svg')
plt.show()
